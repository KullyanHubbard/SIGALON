...
export function KartuJelajah({
  ...
  return (
    <Link
      to={ke}
      className={cn(
        'focus-ring group flex h-full w-full flex-row items-center gap-3.5 rounded-xl border-1 border-black bg-surface p-3 shadow-sm transition-all hover:border-brand-200 hover:shadow-md sm:flex-col sm:items-start sm:gap-0 sm:p-6 sm:hover:-translate-y-1 sm:hover:border-brand-200 sm:hover:shadow-lg',
        className,
      )}
    >
      <div className="shrink-0">{ikon}</div>
      <div className="min-w-0 flex-1 sm:mt-4 sm:flex sm:flex-col sm:w-full">
