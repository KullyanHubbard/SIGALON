import { Fragment, useEffect, useState } from 'react';
import { CHART_KATEGORI_COLORS } from '@/lib/colors';
import type { Distribusi } from '@/types/statistik';

const LEBAR_BAR_MINIMUM_PERSEN = 2;

function lebarBar(nilai: number, maks: number): string {
  if (nilai <= 0) return '0%';
  return `${Math.max((nilai / maks) * 100, LEBAR_BAR_MINIMUM_PERSEN)}%`;
}

export function DistribusiBarChart({ data }: { data: Distribusi[] }) {
  const [terpasang, setTerpasang] = useState(false);

  useEffect(() => {
    const timer = setTimeout(() => {
      setTerpasang(true);
    }, 50);
    return () => clearTimeout(timer);
  }, [data]);

  if (data.length === 0) {
    return <p className="text-sm text-slate-500">Belum ada data.</p>;
  }

  const maks = Math.max(...data.map((d) => d.value), 1);

  return (
    <dl className="grid grid-cols-[auto_1fr_auto] items-center gap-x-2.5 gap-y-3 sm:gap-x-4 sm:gap-y-3.5">
      {data.map((d, i) => (
        <Fragment key={d.label}>
          <dt
            className="max-w-[130px] truncate text-xs font-semibold text-slate-900 sm:max-w-none sm:whitespace-normal sm:text-sm"
            title={d.label}
          >
            {d.label}
          </dt>
          <dd
            className="h-2.5 overflow-hidden rounded-sm bg-slate-100"
            aria-hidden
          >
            <div
              className="h-full rounded-sm transition-[width] duration-700 ease-out motion-reduce:transition-none"
              style={{
                width: terpasang ? lebarBar(d.value, maks) : '0%',
                transitionDelay: `${Math.min(i * 45, 350)}ms`,
                backgroundColor:
                  CHART_KATEGORI_COLORS[i % CHART_KATEGORI_COLORS.length],
              }}
            />
          </dd>
          <dd
            className="text-right text-xs font-bold tabular-nums text-slate-900 transition-opacity duration-500 ease-out motion-reduce:transition-none sm:text-sm"
            style={{
              opacity: terpasang ? 1 : 0,
              transitionDelay: `${Math.min(i * 45 + 100, 450)}ms`,
            }}
          >
            {d.value}
          </dd>
        </Fragment>
      ))}
    </dl>
  );
}
